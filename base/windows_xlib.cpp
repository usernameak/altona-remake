...
int sFont2D::GetWidth(const sChar *text,int len)
{
  if(len==-1) len=sGetStringLen(text);
  XGlyphInfo extents;
  XftTextExtents32(sXDisplay(),prv->Font,(const FcChar32*) text/*,Endianess*/,len/**2*/,&extents);
  return extents.xOff;
}
...
void sFont2D::PrintBasic(int flags,const sRect *r,int x,int y,const sChar *text,int len)
{
  ...
  XftDrawString32(XDraw,&ColorXFT[prv->TextColor],prv->Font,x,y+prv->Font->ascent,(const FcChar32 *)text/*,Endianess*/,len/**2*/);
  if(r)
    sClipPop();
}
