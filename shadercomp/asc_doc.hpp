...
public:
  ...
  sInt RegisterNum;
  sInt Usages;
  bool Function;                 // this is actually an ACFunc
...
struct ACPermuteMember
{
  ...
  bool Group;            // 1:group, 0:element in group
};
...
private:
  ...
  void AddDefTypes(const sChar *name,sInt type,bool many);
  ...
  void OutBinary(ACExpression *left,ACExpression *right,const sChar *name,bool brackets);
  void OutExpr(ACExpression *expr,bool brackets=0);
...
public:
  ...
  bool Parse(const sChar *source); // parse a shader or cbuffer/permute source
  ...
  bool IsValidPermutation(ACPermute *perm,sInt n);
};
